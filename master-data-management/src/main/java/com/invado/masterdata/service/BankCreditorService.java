...
public class BankCreditorService {
    ...
    public BankCreditor create(BankCreditor a) throws com.invado.masterdata.service.exception.IllegalArgumentException,
        ...
        try {
            if (dao.find(BankCreditor.class, a.getId()) != null) {
                throw new javax.persistence.EntityExistsException(
                        Utils.getMessage("BankCreditor.EntityExistsEx", a.getId())
                );
            }
    ...
    private Long count(
        ...
        if (id != null ) {
            criteria.add(cb.equal(cb.upper(root.get(BankCreditor_.id)),
