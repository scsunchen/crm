...
public class ReceivablePayableCardTest {
    ...
    @Inject
    private ReceivablePayableCard service;
    ...
    @Test
    public void testReadForeignCurrencyReceivablePayableCard() throws Exception {
        ...
        Account account = new Account("202000", "", AccountDetermination.CUSTOMERS,
        ...
        BusinessPartner businessPartner = new BusinessPartner("1");
        businessPartner.setId(1);
        Analytical transaction1 = new Analytical();
        ...
        transaction1.setJournalEntryItemOrdinalNumber(1);
        transaction1.setAccount(account);
        transaction1.setValueDate(LocalDate.of(2015, Month.JANUARY, 20));
        transaction1.setRecordDate(LocalDate.of(2015, Month.JANUARY, 15));
        transaction1.setJournalEntryType(type);
        transaction1.setJournalEntryNumber(1);
        transaction1.setCreditDebitRelationDate(LocalDate.of(2015, Month.JANUARY, 15));
        transaction1.setOrgUnit(unit);
        transaction1.setPartner(businessPartner);
        transaction1.setDescription(description);
        
        Analytical transaction2 = new Analytical();
        ...
        transaction2.setJournalEntryItemOrdinalNumber(1);
        transaction2.setAccount(account);
        transaction2.setValueDate(LocalDate.of(2015, Month.JANUARY, 20));
        transaction2.setRecordDate(LocalDate.of(2015, Month.JANUARY, 15));
        ...
        transaction2.setCreditDebitRelationDate(LocalDate.of(2015, Month.JANUARY, 15));
        transaction2.setOrgUnit(unit);
        transaction2.setPartner(businessPartner);
        ...
        ExchangeRate exRate = new ExchangeRate(new Date(), 
                new Currency(DTO.getForeignCurrencyISOCode()));
        exRate.setMiddle(BigDecimal.valueOf(112.0000));
        ...
        Join<Analytical, Account> accountJoin = mocks.createMock(Join.class);
        Join<Analytical, BusinessPartner> partnerJoin = mocks.createMock(Join.class);
        ...
        expect(EM.find(Client.class, DTO.getClientID()))
                .andReturn(new Client());
        expect(EM.getCriteriaBuilder()).andReturn(cb);
        expect(cb.createQuery(Analytical.class)).andReturn(cQuery);
        ...
        expect(root.join(Analytical_.account, JoinType.LEFT))
                .andReturn(accountJoin);
        expect(root.join(Analytical_.partner, JoinType.LEFT))
                .andReturn(partnerJoin);        
        expect(root.get(Analytical_.determination)).andReturn(path);
        expect(cb.equal(path, AccountDetermination.CUSTOMERS))
                .andReturn(predicate);        
        ...
        expect(cb.and(new Predicate[]{predicate, predicate})).andReturn(predicate);
        expect(cQuery.where(predicate)).andReturn(cQuery);
        expect(partnerJoin.get(BusinessPartner_.companyIdNumber)).andReturn(path);
        expect(cb.asc(path)).andReturn(order);
        expect(accountJoin.get(Account_.number)).andReturn(path);
        expect(cb.asc(path)).andReturn(order);
        expect(root.get(Analytical_.recordDate)).andReturn(path);
        expect(cb.asc(path)).andReturn(order);
        ...
        expect(EM.find(Account.class, account.getNumber()))
                .andStubReturn(account);
        expect(EM.find(BusinessPartner.class, businessPartner.getId()))
                .andStubReturn(businessPartner);
        ...
        expect(EM.find(ExchangeRate.class, new ExchangeRatePK(
                ...
                .andStubReturn(exRate);        
        
        mocks.replay();
        ReadLedgerCardsDTO result = service.readReceivablePayableCard(DTO);
        mocks.verify();
        ...
        Assert.assertEquals(null, actual.accountName);
        Assert.assertEquals(null, actual.accountNumber);
        Assert.assertEquals(null, actual.orgUnitID);
        Assert.assertEquals(businessPartner.getCompanyIdNumber(), 
                            actual.partnerRegistrationNumber);
        Assert.assertEquals(null, actual.valueDateFrom);
        Assert.assertEquals(null, actual.valueDateTo);
        Assert.assertEquals(LedgerCardTypeDTO.CUSTOMER, actual.type);
        ...
        Assert.assertEquals(0, Integer.valueOf(1).compareTo(actual.numberOfPages));        

        Assert.assertEquals(2, actual.items.size());
        LedgerCardItemDTO item1Actual = actual.items.get(0);
        Assert.assertEquals(account.getNumber(), item1Actual.accountNumber);
        Assert.assertEquals(0, BigDecimal.valueOf(446.43).compareTo(item1Actual.debit));
        Assert.assertEquals(0, BigDecimal.ZERO.compareTo(item1Actual.credit));
        Assert.assertEquals(0, BigDecimal.valueOf(446.43).compareTo(item1Actual.balance));
        ...
        Assert.assertEquals(transaction1.getJournalEntryItemOrdinalNumber(), 
                            item1Actual.ordinal);
        ...
        Assert.assertEquals(account.getNumber(), item2Actual.accountNumber);
        Assert.assertEquals(0, BigDecimal.valueOf(460.71).compareTo(item2Actual.debit));
        Assert.assertEquals(0, BigDecimal.ZERO.compareTo(item2Actual.credit));
        Assert.assertEquals(0, BigDecimal.valueOf(907.14).compareTo(item2Actual.balance));
        ...
        Assert.assertEquals(transaction2.getJournalEntryItemOrdinalNumber(), 
                            item2Actual.ordinal);
